...
export const scriptedRun =
  ...
  (sessionId, spec, { emit, canUseTool, askQuestion, proposePlan, registerBackgroundStop }) =>
    Effect.gen(function* () {
      ...
      // digest prompt is generated by Starbase, not typed by a user, so there is
...
\`\`\``
      ...
      // Full adversarial-planning E2E fixture. These prompts are generated by
      // Starbase itself (not user text), so recognising their opening contract
      // drives the same proposal → critique → resolver path without a real
      // model or a production-only RPC hook.
      if (spec.prompt.startsWith("You are planning a change to this repository.")) {
        yield* emit({
          _tag: "Assistant",
          text: `\`\`\`plan
...
Add the existing limiter to the refund route and verify its rejection path.`
      ...
      if (spec.prompt.startsWith("You are reviewing a plan another model wrote")) {
        yield* emit({
          _tag: "Assistant",
          text: '```json\n{ "challenges": [] }\n```'
        })
