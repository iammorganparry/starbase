...
export const scriptedPlan = (sessionId: string, rev: number): Plan => ({
  ...
  steps: [
    { id: "s_01", number: "01", title: "Audit session middleware", intent: "See how sessions read tokens today.", approach: ["Read session.ts", "Trace the token path"], kind: "step", condition: null, parentId: null, dependsOn: [], blocks: [], files: [{ path: "src/auth/memory-store.ts", change: "M", added: 0, removed: 0 }], guards: [], diff: null, status: "proposed", flagged: false },
    { id: "s_02", number: "02", title: "Create TokenStore module", intent: "A dedicated store for token lifecycle.", approach: ["Add token-store.ts", "Expose get/set/refresh"], kind: "step", condition: null, parentId: null, dependsOn: ["01"], blocks: [], files: [{ path: "src/auth/token-store.ts", change: "A", added: 40, removed: 0 }], guards: [{ text: "Store is covered by tests", status: "ok" }], diff: { added: 40, removed: 0 }, status: "proposed", flagged: false },
      ...
      if (spec.prompt.includes("[[plan]]") || spec.mode === "plan") {
        ...
        while (true) {
          ...
          if (decision._tag === "Approve") {
            yield* emit({ _tag: "Assistant", text: "Plan approved — starting on step 1." })
            yield* pause
            yield* emit({ _tag: "ToolStart", id: "plan-edit-1", name: "Write", target: "src/auth/token-store.ts" })
            yield* pause
            yield* emit({ _tag: "ToolEnd", id: "plan-edit-1", status: "success", meta: null, diff: { added: 40, removed: 0 }, preview: "1  + export class TokenStore {" })
            break
          }
